...
def data_compare(source_df, target_df, key_column,num_records=10):
    ...
    if failed_count > 0:
        failed2 = failed.select(key_column).distinct().withColumn("hash_key",
                                                                  sha2(concat(*[col(c) for c in key_column]), 256))
        source_df = source_df.withColumn("hash_key", sha2(concat(*[col(c) for c in key_column]), 256)). \
            join(failed2, ["hash_key"], how='left_semi').drop('hash_key')
        target_df = target_df.withColumn("hash_key", sha2(concat(*[col(c) for c in key_column]), 256)). \
            join(failed2, ["hash_key"], how='left_semi').drop('hash_key')
        ...
        print("keycolumns", key_column) #['customer_id']
        for column in columnList: # column = Phone
            if column not in key_column: # email not in ['customer_id'] # True
                key_column.append(column) # ['customer_id','email']
                temp_source = source_df.select(key_column).withColumnRenamed(column, "source_" + column)

                temp_target = target_df.select(key_column).withColumnRenamed(column, "target_" + column)
                key_column.remove(column) # ['customer_id']
